...
##Command line structure: Rscript chi-square-test.R <GLS-table> <GQS-table>
...
result_count_tr1 <- merged_data %>%
  filter(
    ...
    # Criteria 3: New GQS column (abs_df_GQS) is 0.02 or greater
    abs_df_GQS >= 0.02
  ) %>%
...
result_count_tr2 <- merged_data %>%
  filter(
    # Criteria 1: Tree support in BOTH files must be "tr2"
...
cat("  - Absolute delta in GQS ('abs_df_GQS') is >= 0.0.02\n")
